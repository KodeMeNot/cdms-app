let JsonView = (function (exports) {
  ...
  function setCaretIconDown(node) {
    if (node.children.length > 0) {
      let icon = node.el.querySelector('.fas');
  ...
  function createNodeElement(node) {
    ...
    let getSizeString = function getSizeString(node) {
    ...
    if (node.children.length > 0) {
      ...
      let caretEl = el.querySelector('.caret-icon');
  ...
  function renderJSON(jsonData, targetElement) {
    ...
    let tree = createTree(parsedData);
    render(tree, targetElement);
    return tree;
  }

  function render(tree, targetElement) {
    let containerEl = createContainerElement();
